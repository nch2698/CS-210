...
void groupInfoMenu() {
    ...
    switch (choice) {
        ...
        case '3': {
            ...
            int groupName;
            assertInt(groupName, 1, currentNumberOfGroups);// chech for type and range of input
            printLabels();
...
void enterGroupsInfo(Group **oldGroupsLoc) {
    ...
    for (int i = 0; i < groupNum; ++i) {
        ...
        for (int j = 0; j < studentNum; ++j) {
            ...
            cout << "- Student " << j + 1 << ":\n";
            cout << "\tStudent\'s name:\t";
            assertStringEmpty(s.name); //	input & check for empty string 
 			/*
 			input & check for duplicated name 
 			including the case students have the same name
 			*/
...
void editDueDate() {
    ...
    Date date = {};
    assertDate(date); // input and check for date validation
...
void statisticsOfAProject() {
    ...
    cout << BAR << "Project "
         << left << setw(7) << proj;
    for (int i = 0; i < currentNumberOfGroups; ++i) {
        // projectID fixes, groupID changes
		string res = "NOT YET";
...
void statisticsOfAGroup() {
    ...
    for (int i = 0; i < currentNumberOfProjects; ++i) {
        // projectID changes, groupID fixes 
		string res = "NOT YET";
...
void overallStatisticsForAPeriod(Date &finalDate) {
    ...
    bool validProject[MAX_PROJECTS_NUMBER] = {false}; // later used to check validation for each project 
    int n = 0;
    for (int i = 0; i < currentNumberOfProjects; ++i) {
    ...
    for (int groupID = 0; groupID < currentNumberOfGroups; ++groupID) { // print all groups
        ...
        for (int i = 0; i < currentNumberOfProjects; ++i) { 
            if (validProject[i]) { // only print validated projects
                string res = "NOT YET";
